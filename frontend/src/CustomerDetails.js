...
export default function CustomerDetails() {
  ...
  useEffect(() => {
    Axios.get("http://localhost:9000/customerDetails")
      .then((res) => {
  ...
  function deleteUser(id){
    fetch(`http://localhost:9000/customerDetails/${id}`,{
      method: 'DELETE'
    }).then((result) => {
      ...
      <Table sx={{ minWidth: 700 }} aria-label="customized table">
        ...
        <TableBody>
        {
            posts.filter((post) => {
              if (searchItems == ""){
                return post
              }else if (post.customerName.toLowerCase().includes(searchItems.toLowerCase())){
                return post
              }
            })
          
          .map((post) => (
            <StyledTableRow key={post.customerName}>
